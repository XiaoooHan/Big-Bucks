...
public class DBUtil {
	...
	private void initDB() throws SQLException {
		...
		statement.execute("CREATE TABLE PORTFOLIO (STOCK_ID INTEGER NOT NULL GENERATED ALWAYS AS IDENTITY (START WITH 1, INCREMENT BY 1), ACCOUNTID BIGINT NOT NULL,SYMBOL VARCHAR(100) NOT NULL, AMOUNT BIGINT NOT NULL, AVGPRICE DOUBLE NOT NULL, VALUE DOUBLE NOT NULL,PRIMARY KEY (TRADING_ID))");
	...
	public static String setPortfolio(long accountID,long stockID, String symbol, double price, int amount,String action) throws SQLException{
		Connection connection = getConnection();
		Statement statement = connection.createStatement();

		ResultSet resultSet =statement.executeQuery("SELECT COUNT(*)FROM PORTFOLIO WHERE SYMBOL = '" + symbol + "'");
