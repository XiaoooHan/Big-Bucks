...
public class DBUtil {
	...
	private void initDB() throws SQLException {
		...
		statement.execute("CREATE TABLE PEOPLE (USER_ID VARCHAR(50) NOT NULL UNIQUE, PASSWORD VARCHAR(20) NOT NULL, FIRST_NAME VARCHAR(100) NOT NULL, LAST_NAME VARCHAR(100) NOT NULL, ROLE VARCHAR(50) NOT NULL, PRIMARY KEY (USER_ID))");
	...
	public static Transaction[] getTransactions(String startDate, String endDate, Account[] accounts, int rowCount) throws SQLException {
		...
		while (resultSet.next()){
			...
			String symbol = resultSet.getString("SMBL");
			Timestamp date = resultSet.getTimestamp("DATE");
			String desc = resultSet.getString("TYPE");
			double amount = resultSet.getDouble("AMOUNT");
			double price = resultSet.getDouble("AMOUNT");
			transactions.add(new Transaction(transId, actId,symbol, desc, amount, date,price));
		}
